...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const iframeElement = document.getElementById('mapIframe');
    const buttons = document.querySelectorAll('.filter-btn');

    // Base directory untuk file-file peta Anda
    const BASE_DIR = '../HTML/'; 

    // Objek pemetaan untuk menentukan nama file yang benar
    const fileMap = {
        'All': 'routingAll.html',
        'Nature': 'RoutingNature2.html',
        'Culture': 'RoutingCulture2.html',
        'Recreation': 'RoutingRecreation2.html',
        'Culinary': 'RoutingCulinary2.html' 
        // Pastikan Anda memiliki file-file ini di direktori ../HTML/
    };
    ...
    filterContainer.addEventListener('click', function(event) {
        ...
        if (clickedButton) {
            ...
            // 1. Perbarui tampilan tombol (aktif/tidak aktif)
            buttons.forEach(btn => btn.classList.remove('active'));
            clickedButton.classList.add('active');

            // 2. Tentukan nama file
            const fileName = fileMap[filterValue];
            
            if (fileName) {
                // 3. Perbarui atribut src pada iframe
                iframeElement.src = BASE_DIR + fileName;
                console.log('Mengubah iframe ke:', iframeElement.src);
            } else {
                console.error('File peta tidak ditemukan untuk filter:', filterValue);
            }
        }
    });
});
